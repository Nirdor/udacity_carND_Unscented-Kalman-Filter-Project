...
UKF::UKF() {
  ...
  */
  ...
  //percentage of Radar NIS values exceeding the 95% line - should be 0.5
  nis_radar_ = 0.0;

  //percentage of Lidar NIS values exceeding the 95% line - should be 0.5
  nis_lidar_ = 0.0;
  ...
  if (!is_initialized_) {
    if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
      x_ << meas_package.raw_measurements_(0) * cos(meas_package.raw_measurements_(1)),
            ...
            abs(meas_package.raw_measurements_(2)), //Lower bound for velocity
            0,
            0;
    }
...
void UKF::UpdateLidar(MeasurementPackage meas_package, MatrixXd Xsig_pred) {
  ...
  */
  ...
  if (epsilon > 5.991) {
    nis_lidar_ = (nis_lidar_ * (count_lidar_ - 1) + 1) / count_lidar_;
  } else {
    nis_lidar_ = nis_lidar_ * (count_lidar_ - 1) / count_lidar_;
  }
  cout << "Lidar NIS: " << nis_lidar_ << endl; //should be 0.5
}
...
void UKF::UpdateRadar(MeasurementPackage meas_package, MatrixXd Xsig_pred) {
  ...
  */
  ...
  double epsilon = z_diff.transpose() * S.inverse() * z_diff;
  if (epsilon > 7.815) {
    nis_radar_ = (nis_radar_ * (count_radar_ - 1) + 1) / count_radar_;
  } else {
    nis_radar_ = nis_radar_ * (count_radar_ - 1) / count_radar_;
  }
